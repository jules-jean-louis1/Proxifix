...
export const AdminEquipmentForm: React.FC<AdminEquipmentFormProps> = ({
  ...
  const [typeEquipments, setTypeEquipments] = useState<{ label: string; value: number }[]>([]);
  const [operatingSystems, setOperatingSystems] = useState<{ label: string; value: number }[]>([]);
  ...
  const loadOptions = async () => {
    try {
      ...
      const typeEquipmentsOptions = typeEquipmentsResponse.data.map((type: any) => ({
        label: type.name,
        value: type.id,
      }));
    ...
    <View>
      <FormProvider {...methods}>
        ...
        <AppSimpleSearchInput
          ...
          displayKey={(item) => `${item.first_name} ${item.last_name} (${item.email})`}
          valueKey="id"
