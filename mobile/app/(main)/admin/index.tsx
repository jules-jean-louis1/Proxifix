        ...
        return (
            <View style={{ flex: 1 }}>
                <ScrollView style={styles.container}>
                    ...
                    <Text style={styles.subtitle}>Vos interventions assignées ({interventions.length})</Text>
                    ...
                    {interventions.length > 0 ? (
                        interventions.map((intervention: any) => (
                            <View key={intervention.id} style={styles.interventionCard}>
                                ...
                                <Text variant="bodySmall">Client: {intervention.customer?.first_name} {intervention.customer?.last_name}</Text>
                                <Text variant="bodySmall">Date: {new Date(intervention.start_date).toLocaleDateString()}</Text>
                            </View>
                        ))
                    ) : (
        ...
        return (
            <View style={{ flex: 1 }}>
                <ScrollView style={styles.container}>
                    ...
                    <Text style={styles.subtitle}>Dashboard Admin - {sessionData?.company?.name}</Text>
                    ...
                    {interventions.slice(0, 5).map((intervention: any) => (
                        <View key={intervention.id} style={styles.interventionCard}>
                            <Text variant="titleMedium">{intervention.title}</Text>
                            <Text variant="bodyMedium">Status: {intervention.status}</Text>
                            <Text variant="bodySmall">Technicien: {intervention.technician?.first_name || 'Non assigné'}</Text>
                            <Text variant="bodySmall">Client: {intervention.customer?.first_name} {intervention.customer?.last_name}</Text>
                        </View>
                    ))}
