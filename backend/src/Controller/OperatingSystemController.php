...
final class OperatingSystemController extends AbstractController
{
    ...
    public function getOperatingSystems(
        ...
        if ($id) {
            ...
            return $this->json(
                ...
                Response::HTTP_OK,
                [],
                ['groups' => ['operatingSystem.get_one']],
            );
        }

        if ($name) {
            $operatingSystems = $operatingSystemRepository->findBy([
                'name' => $name,
            ]);
            if (! $operatingSystems) {
                return $this->json(
                    ['error' => 'No Operating Systems found with that name'],
                    Response::HTTP_NOT_FOUND,
                );
            }
        } else {
        ...
        if (! $operatingSystems) {
            return $this->json(
                ['error' => 'No Operating Systems found'],
                Response::HTTP_NOT_FOUND,
            );
        }
    ...
    public function create(
        ...
        #[
            MapRequestPayload(
                serializationContext: ['groups' => 'operatingSystem.create '],
            ),
        ]
    ...
    public function update(
        ...
        $operatingSystem = $em
            ->getRepository(OperatingSystem::class)
            ->find($id);
        if (! $operatingSystem) {
            return $this->json(
                ['error' => 'Operating System not found'],
