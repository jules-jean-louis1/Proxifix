...
sequenceDiagram
    ...
    AppMobile->>Serveur: GET /companies/{id}/slots?date={...}
    Serveur->>BDD: CALL get_free_slots(date, company_id)
    BDD-->>Serveur: Liste des créneaux disponibles
    Serveur-->>AppMobile: Liste créneaux
    AppMobile->>Utilisateur: Affiche les créneaux disponibles

    Utilisateur->>AppMobile: Choisit un créneau
    AppMobile->>Serveur: POST /appointment_requests
    Serveur->>BDD: INSERT INTO appointment_request (user_id, company_id, equipment_id, type_intervention_id, date, status)
    BDD-->>Serveur: OK
    Serveur-->>AppMobile: RDV en attente de validation
    AppMobile-->>Utilisateur: Confirmation envoyée
...
sequenceDiagram
    ...
    Technicien->>Frontend: Connexion au dashboard
    Frontend->>API: GET /interventions/{id}
    API->>DB: SELECT * FROM intervention WHERE id = ?
    DB-->>API: Intervention + tâches associées
    API-->>Frontend: Détails de l’intervention

    Technicien->>Frontend: Valide l’intervention
    Frontend->>API: PATCH /interventions/{id}/validate
    API->>DB: UPDATE intervention SET status_id = COMPLETED, updated_at = NOW()
    DB-->>API: OK
    API-->>Frontend: Intervention validée
```

✅ Édition d’une entreprise par un administrateur :
...
sequenceDiagram
    ...
    Admin->>Frontend: Accède à la fiche entreprise
    Frontend->>API: GET /companies/{id}
    API->>DB: SELECT * FROM company WHERE id = ?
    DB-->>API: Données entreprise
    API-->>Frontend: Affiche les infos

    Admin->>Frontend: Modifie les données (nom, adresse, etc.)
    Frontend->>API: PUT /companies/{id}
    API->>DB: UPDATE company SET ... WHERE id = ?
    DB-->>API: OK
    API-->>Frontend: Confirmation de mise à jour
```

✅ Création d’un équipement (avec type, marque, OS)
...
sequenceDiagram
    ...
    Utilisateur->>Frontend: Clique "Ajouter un équipement"
    Frontend->>API: GET /brands, /operating-systems, /equipment-types
    API->>DB: SELECT * FROM brand / os / type_equipment
    DB-->>API: Données listes
    API-->>Frontend: Liste déroulante

    Utilisateur->>Frontend: Remplit formulaire équipement
    Frontend->>API: POST /equipments
    API->>DB: INSERT INTO equipment (user_id, brand_id, type_equipment_id, operating_system_id, name)
    DB-->>API: OK
    API-->>Frontend: Équipement enregistré
```

✅ Demande de création d’entreprise par un utilisateur
...
sequenceDiagram
    ...
    Utilisateur->>Frontend: Remplit le formulaire de création d’entreprise
    Frontend->>API: POST /register-company
    API->>DB: INSERT INTO user (...) INSERT INTO company (name, ..., is_approved = null)
    DB-->>API: OK
    API-->>Frontend: Demande envoyée

    SuperAdmin->>Frontend: Connexion avec rôle SUPER_ADMIN
    Frontend->>API: GET /admin/pending-companies
    API->>DB: SELECT * FROM company WHERE is_approved IS NULL
    DB-->>API: Liste entreprises en attente
    API-->>Frontend: Affiche la liste
    SuperAdmin->>Frontend: Valide une entreprise
    Frontend->>API: PATCH /admin/companies/{id}/approve
    API->>DB: UPDATE company SET is_approved = true, approved_by_id = {superadmin_id}, approved_at = NOW()
    DB-->>API: OK
    API-->>Frontend: Entreprise validée
```
